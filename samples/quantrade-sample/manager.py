...
class FoolManager(PortfolioManager):
    ...
    def optimize(self, date, to_buy, to_sell, parameters):
        ...
        allocations = {}
        for s in to_sell:
            allocations[s] = - int(random.random() * parameters.get('sell_scale', 100))
            self.log.info('Selling {} of {}'.format(allocations[s], s))

        for s in to_buy:
            allocations[s] = int(random.random() * parameters.get('buy_scale', 100))
            self.log.info('Buying {} of {}'.format(allocations[s], s))
